...
void base_socket::close() {
   int status = ::close (socket_fd); //:: NOT use base_socket::close
...
void base_socket::set_non_blocking (const bool blocking) {
   ...
   opts = ::fcntl (socket_fd, F_SETFL, opts); //function overloading!!
   if (opts < 0) throw socket_sys_error ("fcntl");
}
...
// private to_string for base_socket
